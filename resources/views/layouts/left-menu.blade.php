        ...
        <ul class="nav flex-column pt-2 w-100">
            ...
            <li class="nav-item">
                ...
                <a class="nav-link d-flex {{ request()->is('classes') ? 'active' : '' }}"
                    href="{{ route('class.list') }}"><i class="bi bi-diagram-3"></i> <span
            ...
            @if (Auth::user()->role != 'student')
                <li class="nav-item">
                    ...
                    <ul class="nav collapse {{ request()->is('students*') ? 'show' : 'hide' }} bg-white"
                        id="student-submenu">
                        <li class="nav-item w-100"><a class="nav-link" href="{{ route('student.list') }}"><i
                                    class="bi bi-person-video2 me-2"></i> View Students</a></li>
                        {{-- @if (!session()->has('browse_session_id') && Auth::user()->role == 'admin') --}}
                        <li class="nav-item w-100"><a class="nav-link" href="{{ route('student.create') }}"><i
                    ...
                    <ul class="nav collapse {{ request()->is('teachers*') ? 'show' : 'hide' }} bg-white"
                        ...
                        <li class="nav-item w-100"
                            {{ request()->routeIs('teacher.list.show') ? 'style="font-weight:bold;"' : '' }}><a
                                class="nav-link" href="{{ route('teacher.list') }}"><i
                                    class="bi bi-person-video2 me-2"></i> View Teachers</a></li>
                        {{-- @if (!session()->has('browse_session_id') && Auth::user()->role == 'admin') --}}
                        <li class="nav-item w-100"
                            {{ request()->routeIs('teacher.create') ? 'style="font-weight:bold;"' : '' }}><a
                                class="nav-link" href="{{ route('teacher.create') }}"><i
            ...
            @if (Auth::user()->role == 'teacher')
                <li class="nav-item">
                    <a class="nav-link {{ request()->is('courses/teacher*') || request()->is('courses/assignments*') ? 'active' : '' }}"
                        href="{{ route('course.teacher.list', ['teacher_id' => Auth::user()->id]) }}"><i
            ...
            @if (Auth::user()->role == 'student')
                <li class="nav-item">
                    <a class="nav-link " href="{{ route('student.attendance', ['id' => Auth::user()->id]) }}"><i
                            class="bi bi-calendar2-week"></i> <span
                            class="ms-1 d-inline d-sm-none d-md-none d-xl-inline">Attendance</span></a>
                <li class="nav-item">
                    <a class="nav-link "
                        href="{{ route('course.student.list', ['student_id' => Auth::user()->id]) }}"><i
                            class="bi bi-journal-medical"></i> <span
            ...
            @if (Auth::user()->role != 'student')
                <li class="nav-item border-bottom">
                    ...
                    <ul class="nav collapse {{ request()->is('exams*') ? 'show' : 'hide' }} bg-white"
                        ...
                        <li class="nav-item w-100"><a class="nav-link" href="{{ route('exam.list') }}"><i
                                    class="bi bi-file-text me-2"></i> View Exams</a></li>
                        @if (Auth::user()->role == 'admin' || Auth::user()->role == 'teacher')
                            <li class="nav-item w-100"><a class="nav-link" href="{{ route('exam.create') }}"><i
                        ...
                        @if (Auth::user()->role == 'admin')
                            <li class="nav-item w-100"><a class="nav-link"
                                    ...
                                    Add Grade Systems</a></li>
                        @endif
                        <li class="nav-item w-100"><a class="nav-link" href="{{ route('exam.grade.system.list') }}"><i
            ...
            @if (Auth::user()->role == 'admin')
                <li class="nav-item">
                    <a class="nav-link {{ request()->is('academics*') ? 'active' : '' }}"
                        href="{{ route('academic.setting') }}"><i class="bi bi-tools"></i> <span
            ...
            @if (!session()->has('browse_session_id') && Auth::user()->role == 'admin')
                <li class="nav-item">
                    <a class="nav-link {{ request()->is('promotions*') ? 'active' : '' }}"
                        href="{{ route('promotions.list') }}"><i class="bi bi-sort-numeric-up-alt"></i> <span
